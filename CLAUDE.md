...
Companion docs: `Design Documentation/architecture.md` (class model, pipeline),
`Design Documentation/design.md` (requirements with FR/NFR ids). Follow those;
this brief tells you how to build against them and in what order.
...
## Hardware note (resolves the M1 vendor:product risk)

The actual pedal is a **PCsensor FootSwitch, VID `3553` PID `B001`**. It is
keyboard/mouse-emulating, not raw-HID: it enumerates as a composite device
whose keyboard and mouse collections are owned exclusively by Windows'
kbdhid.sys/mouhid.sys, so `hidapi` reads on those collections fail
immediately (confirmed by probing). `devices/hid_pedal.py` reads it via the
Windows Raw Input API (`devices/raw_input.py`, ctypes, stdlib only) instead:
each `WM_INPUT` message carries the originating device's HANDLE, which
resolves to the same underlying interface `hid.enumerate()` reports (through
a different device-interface-class GUID; `devices/base.py:normalize_hid_path`
reconciles the two). This still does not suppress the pedal's own
keystroke/click from also reaching the focused window; that is grab and
remains deferred per the guardrails below. `devices/base.py` still uses
`hidapi` for enumeration/listing (FR-1) since that part works fine; only
*reading* the OS-claimed collections needed the Raw Input path.
